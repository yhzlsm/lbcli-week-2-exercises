...
SPEND_AMOUNT=$(echo "$PAYMENT_AMOUNT + $FEE_SATS" | bc -l)
CHANGE_AMOUNT=$(echo "$UTXO_VALUE - $SPEND_AMOUNT" | bc)
check_cmd "Change calculation" "CHANGE_AMOUNT" "$CHANGE_AMOUNT"

# Convert amounts to BTC for createrawtransaction
PAYMENT_BTC=$(echo "scale=8; $PAYMENT_AMOUNT / 100000000" | bc -l | awk '{printf "%.8f\n", $0}')
CHANGE_BTC=$(echo "scale=8; $CHANGE_AMOUNT / 100000000" | bc -l | awk '{printf "%.8f\n", $0}')
...
CHILD_SEND_AMOUNT=$(echo "$PAYMENT_AMOUNT - $CHILD_FEE_SATS" | bc)
check_cmd "Child amount calculation" "CHILD_SEND_AMOUNT" "$CHILD_SEND_AMOUNT"

# Convert to BTC
CHILD_SEND_BTC=$(echo "scale=8; $CHILD_SEND_AMOUNT / 100000000" | bc -l | awk '{printf "%.8f\n", $0}')
...
# WRITE YOUR SOLUTION BELOW:
TIMELOCK_INPUTS='[{"txid":"'$PARENT_TXID'","vout":'$CHANGE_OUTPUT_INDEX', "sequence":'167772160'}]'
...
# Convert to BTC
TIMELOCK_BTC=$(echo "scale=8; $TIMELOCK_AMOUNT / 100000000" | bc -l | awk '{printf "%.8f\n", $0}')
